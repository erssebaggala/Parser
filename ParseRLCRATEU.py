...
def ParseRLCRATEU (line, listOfLogObj, PREVIOUS_LAT,PREVIOUS_LONG,PREVIOUS_MCC,PREVIOUS_MNC):
    dataOfRLCRATEU = ""
    length = len(line)
    if 2 < length:
        logObj = LogDT()
        ...
        if currentLog.modeSystem == 'LTE TDD' or currentLog.modeSystem == 'LTE FDD':
            ...
            if headerParams > 0:
                if ((5 + NumberOfContextID) < length) and (line[5 + NumberOfContextID] != ''):
                    currentLog.RLCRateUL = int(line[5 + NumberOfContextID])
                if headerParams > 1:
                    if ((6 + NumberOfContextID) < length) and (line[6 + NumberOfContextID] != ''):
                        currentLog.RLCULBlockRate = int(line[6 + NumberOfContextID])
                    if headerParams > 2:
                        if ((7 + NumberOfContextID) < length) and (line[7 + NumberOfContextID] != ''):
                            currentLog.RLCRetransmissionUL = float(line[7 + NumberOfContextID])
                ...
                for p in range(0, numberOfParamsPerRB):
                    ...
                    if ((paramNumber) < length) and (line[paramNumber] != ''):
                        if p == 0:
                            ...
                            currentLog.RLCULBlockRate = int(line[paramNumber])
                        elif p == 3:
        ...
        dataOfRLCRATEU = "No of context id not found"
        return 0
