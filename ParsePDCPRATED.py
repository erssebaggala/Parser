...
def ParsePDCPRATED (line, listOfLogObj, PREVIOUS_LAT,PREVIOUS_LONG,PREVIOUS_MCC,PREVIOUS_MNC):
    dataOfPDCPRATED = ""
    length = len(line)
    if 2 < length:
        logObj = LogDT()
        ...
        if currentLog.modeSystem == 'LTE TDD' or currentLog.modeSystem == 'LTE FDD':
            ...
            if headerParams > 0:
                if ((5 + NumberOfContextID) < length) and (line[5 + NumberOfContextID] != ''):
                    currentLog.PDCPDLBitRate = int(line[5 + NumberOfContextID])
                if headerParams > 1:
                    if ((6 + NumberOfContextID) < length) and (line[6 + NumberOfContextID] != ''):
                        currentLog.PDCPDLBlockRate = int(line[6 + NumberOfContextID])
            numberOfRBs = 0
                ...
                for p in range(0, numberOfParamsPerRB):
                    ...
                    if ((paramNumber) < length) and (line[paramNumber] != ''):
                        if p == 0:
                            ...
                            currentLog.PDCPDLBitRate = int(line[paramNumber])
                        elif p == 2:
                            currentLog.PDCPDLBlockRate = int(line[paramNumber])
